...
function Index() {
  ...
  const sofaCollectionImages = products
    .filter((p) => {
      ...
      return text.includes("sofa") || text.includes("sectional");
    })
  ...
  const bedCollectionImages = products
    .filter((p) => {
      ...
      const subType = normalizeText(p.sub_type);
      const text = `${category} ${subType}`;
      return text.includes("bed") || text.includes("upholstered bed");
    })
    .flatMap(getProductImages);
  ...
  const [collectionImageIndex, setCollectionImageIndex] = useState({
    sofas: 0,
    beds: 0,
    custom: 0,
  });
      ...
      setCollectionImageIndex((current) => ({
        sofas:
          sofaCollectionImages.length > 1
            ? (current.sofas + 1) % sofaCollectionImages.length
            : 0,
        beds:
          bedCollectionImages.length > 1
            ? (current.beds + 1) % bedCollectionImages.length
            : 0,
        custom:
          customCollectionImages.length > 1
            ? (current.custom + 1) % customCollectionImages.length
            : 0,
      }));
                ...
                sofaCollectionImages.map((image, index) => (
                  <img
                    ...
                    className={`absolute inset-0 h-full w-full object-cover transition-opacity duration-[1800ms] ${
                      index === collectionImageIndex.sofas
                        ? "opacity-100"
                        : "opacity-0"
                    }`}
                  />
              ...
              {bedCollectionImages.length > 0 ? (
                bedCollectionImages.map((image, index) => (
                  <img
                    ...
                    loading={index === 0 ? "eager" : "lazy"}
                    className={`absolute inset-0 h-full w-full object-cover transition-opacity duration-[1800ms] ${
                      index === collectionImageIndex.beds
                        ? "opacity-100"
                        : "opacity-0"
                    }`}
                  />
              ...
              {customCollectionImages.length > 0 ? (
                customCollectionImages.map((image, index) => (
                  <img
                    ...
                    loading={index === 0 ? "eager" : "lazy"}
                    className={`absolute inset-0 h-full w-full object-cover transition-opacity duration-[1800ms] ${
                      index === collectionImageIndex.custom
                        ? "opacity-100"
                        : "opacity-0"
                    }`}
                  />
